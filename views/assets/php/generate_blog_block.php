...
function generate_blog_block($block, $full_block = false) {
    ...
    if(array_key_exists("thumbnail_url", $block) && !$full_block) {
        $HTMLToReturn .= " photo'><a class='image_container' href='" . $block_url . "'><img class='lazy_load' src='/api/image_resize.php?p=" . $block["thumbnail_url"] . "' data-src='" . $block["thumbnail_url"] . "' alt='" . $block_title . "'></a>";
    } else {
        $HTMLToReturn .= "'>";
    }
